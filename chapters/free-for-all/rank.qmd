...
sample_zeta <- function(theta, y) {
  ...
  for (r in 1:R) {
    # n_attempts to make sure while() loop doesn't go forever
    n_attempts <- 0
    while(!check_zeta(zeta[r,], y[r,]) & n_attempts < 10) {
      zeta[r,] <- rnorm(A, theta, 1)
    }
    n_attempts <- n_attempts + 1
  }
  
  # Make sure NA entries for y are NA in zeta
  return(zeta * (y > 0))
}
